...
def crossref_lookup(index, authors, titles, submitted, num_threads=1):
    ...
    content = []
    with open(working_folder + "/dois.csv") as f:
        line = []
        tab_count = 0
        while True:
            c = f.read(1)
    ...
    with open(working_folder + "/dois_corrected.csv", "wb") as f:
        for line in content:
            f.write(line+"\n")

    cr_lookup = pd.read_csv(working_folder + "/dois_corrected.csv", sep="\t")
...
def doi_lookup(i, num_workers=1, stage1_dir=None, mode='all'):
    """
